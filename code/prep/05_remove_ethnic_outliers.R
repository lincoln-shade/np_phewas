...
# PCA data
merged <- fread("data/tmp/adc_1000g_merged_pca.eigenvec", header = F) %>% 
  .[, 2:4] %>% 
...
#scree plot
eigenval <- fread("data/tmp/adc_1000g_merged_pca.eigenval", header = F) %>% 
...
scree <- ggplot(eigenval, aes(PC, var.expl)) +
  geom_point() +
  ggtitle("Scree plot for 1000 Genomes and adc subjects") +
...
adc_ids <- fread("data/tmp/np_ids.tmp", header = F)
adc_ids <- adc_ids[V2 %in% adc$sample]

ggsave(filename = "doc/adc_1000g_pca.png", pca.plot2, units="in", width=7, height=7)

write.table(adc_ids, "data/adc_qced.txt", col.names = F, row.names = F, quote = F)
