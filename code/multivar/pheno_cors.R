...
colnames(cor_mat) <- rownames(cor_mat) <- colnames(pheno_4k[, -c("FID", "IID")])
for (i in 1L:n_cols) {
  pheno1 <- colnames(pheno_4k)[i + 2L]
  for (j in i:n_cols) {
    pheno2 <- colnames(pheno_4k)[j + 2L]
    cor_mat[i, j] <- cor_mat[j, i] <- pheno_4k[, cor(get(..pheno1), get(..pheno2), use = "c", method = 's')]
  }
}
