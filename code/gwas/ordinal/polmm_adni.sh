...
mkdir output/gwas/adni/polmm

phenos=$(cat data/mega/adni_np_ord.pheno | \
         cut -d " " -f 3- | \
         awk 'NR == 1{print}')
...
for pheno in $phenos
  ...
  # create null model
  null_model=output/gwas/adni/polmm/"$pheno"_null_model.Rds
  ./code/gwas/ordinal/polmm_null_model.R \
    --covar data/mega/adni_np.covar \
    --pheno data/mega/adni_np_ord.pheno \
  ...
  # run regression
  results=output/gwas/adni/polmm/"$pheno"_polmm_results.Rds
  ...
  -r $results \
  -o output/gwas/adni/polmm/"$pheno"_polmm_results.txt
  done
