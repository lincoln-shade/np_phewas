...
phenos=$(cat data/mega/nacc_np_ord.pheno | \
         cut -d " " -f 3- | \
         awk 'NR == 1{print}')
...
for pheno in $phenos
  ...
  ./code/gwas/ordinal/polmm_null_model.R \
    ...
    --pheno data/mega/nacc_np_ord.pheno \
  ...
  python ./code/gwas/clump_snps.py \
    -r ./output/gwas/adc/polmm/"$pheno"_polmm_results.txt \
    -o output/gwas/adc/polmm/"$pheno"_polmm_results \
    -b data/mega/mega_np \
    --snp_field SNPID \
    --p_field pval.spa
  done
