...
parser$add_argument("-m", "--snp_set_size", 
                    ...
                    default = 50000L)
parser$add_argument("--snp_list", help = "text filename with list of SNP IDs")
...
if (is.null(args$snp_list)) {
  ...
  snp_list <- fread(args$snp_list, header = FALSE, blank.lines.skip = TRUE)
  snp_list <- snp_list$V1
  print(snp_list)
  mat <- read.plink(bed = paste0(plink, '.bed'),
                    bim = paste0(plink, '.bim'),
                    fam = paste0(plink, '.fam'),
                    select.snps = snp_list)
  saveRDS(mat, file = paste0(args$out, "SnpMatrix.Rds"))
}
